---
url: /v3.23/features/command-palette.md
description: >-
...
Backspace on a mode's empty input to back out: the mode closes and
...
* **Backspace on an empty input** backs out one level. At the top, Backspace closes the mode and returns `#` to the input as plain text.
...
* **Backspace on an empty input** steps back one level per press: the filter, then the field it came from, then the bucket. Esc leaves the mode.
...
* Backspace on an empty input pops one level. With an empty stack, it exits the mode and restores the trigger the user typed as plain text.
