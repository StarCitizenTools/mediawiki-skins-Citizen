...
- **Press <kbd>↵</kbd>** to open the highlighted mode.
- **<kbd>Backspace</kbd> on the empty input** leaves help and returns the `?`
  or `/help` you typed to the input as plain text, as it does for any other
  trigger. <kbd>Esc</kbd> clears what you typed, then leaves help.
...
position are kept. Typing, <kbd>Backspace</kbd>, <kbd>Esc</kbd> or the back
button closes it.
